...
const normalizeStatementResponse = (response) => {
  ...
  return {
    ...
    rows,
    pagination,
    party: payload?.supplier || payload?.party || null,
...
export default function SupplierStatement() {
  ...
  const supplierName = party?.name || location.state?.name || `#${id}`;
  const supplierPhone = party?.phone || location.state?.phone || '—';
  ...
  const invoiceItemsColumns = [
    ...
    },
    {
      ...
      label: 'الكمية',
      render: (_, row) => Number(row?.ordered_quantity ?? row?.quantity ?? row?.qty ?? 0).toLocaleString('ar-EG'),
    },
    {
      key: 'unit_price',
      label: 'السعر',
    ...
    },
    {
      ...
      label: 'الإجمالي',
      render: (_, row) => {
        const quantity = Number(row?.received_quantity ?? row?.ordered_quantity ?? row?.quantity ?? row?.qty ?? 0) || 0;
    ...
    <div className="space-y-4 print-area">
      ...
      <div className="screen-only rounded-xl border border-border bg-white p-4 print-card">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <h2 className="text-xl font-bold text-text">كشف حساب: {supplierName}</h2>
        </div>

        <div className="flex flex-wrap items-center gap-4 text-sm text-text-muted">
          <span>الهاتف: {supplierPhone}</span>
