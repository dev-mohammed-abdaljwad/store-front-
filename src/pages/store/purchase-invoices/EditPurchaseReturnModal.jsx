...
import { getPurchaseReturn, updatePurchaseReturn, searchPurchaseInvoices } from '../../../api/purchaseInvoices';
import { getSuppliers } from '../../../api/suppliers';
import SearchableSelect from '../../../components/shared/SearchableSelect';
import { formatCurrency } from '../../../utils/formatters';

const purchaseReturnSchema = z
  .object({
  ...
  .superRefine((values, ctx) => {
    ...
    if ((Number(values.refund_amount) || 0) > total) {
      ctx.addIssue({
        ...
        message: 'المبلغ المسترد لا يمكن أن يتجاوز إجمالي المرتجع',
        path: ['refund_amount'],
      });
    }
  });
...
const searchSuppliers = (search = '') => getSuppliers(1, { per_page: 20, search: search || undefined });

export default function EditPurchaseReturnModal({ returnId, onClose, onSuccess }) {
  const [linkInvoice, setLinkInvoice] = useState(false);

  const { data: response, isLoading } = useQuery({
    queryKey: ['purchase-return-detail', returnId],
    queryFn: () => getPurchaseReturn(returnId),
    enabled: !!returnId,
  });
    ...
    if (returnData) {
      reset({
        ...
        items: (returnData.items || []).map((item) => ({
          ...
          variant: item.variant ? { id: item.variant_id, name: `${item.product_name || ''} - ${item.variant_name || ''}`, purchase_price: item.unit_price } : null,
        })),
      });
      if (returnData.purchase_invoice_id) {
        setLinkInvoice(true);
      }
    }
  }, [returnData, reset]);
  ...
  const updateMutation = useMutation({
    mutationFn: (payload) => updatePurchaseReturn(returnId, payload),
    onSuccess: () => {
      toast.success('تم تعديل مرتجع الشراء بنجاح');
      onSuccess?.();
    },
    onError: (error) => {
      const message =
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-2 sm:p-4">
      <div className="my-4 w-full max-w-4xl rounded-xl bg-white shadow-xl sm:my-8">
        <div className="flex items-start justify-between gap-3 border-b border-border p-4 sm:items-center sm:p-6">
          <div>
            <h2 className="text-lg font-bold text-text">تعديل مرتجع مشتريات</h2>
            <p className="mt-0.5 text-sm text-text-muted">تعديل بيانات البضاعة المرجعة إلى المورد</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 p-4 sm:p-6">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-text">المورد *</label>
            <SearchableSelect
              value={Number(watch('supplier_id')) || null}
              onChange={(id) => setValue('supplier_id', id ?? 0, { shouldValidate: true, shouldDirty: true })}
              fetchFn={searchSuppliers}
              queryKey="purchase-return-supplier"
              placeholder="ابحث عن مورد..."
              renderOption={(supplier) => `${supplier.name}${supplier.phone ? ` - ${supplier.phone}` : ''}`}
              renderSelected={(supplier) => supplier.name}
              initialSelected={returnData?.supplier}
          ...
          <div className="space-y-3 rounded-lg border border-border p-4">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="linkPurchaseInvoice"
                checked={linkInvoice}
                onChange={(event) => {
                  const enabled = event.target.checked;
                  setLinkInvoice(enabled);
                  if (!enabled) setValue('purchase_invoice_id', null, { shouldDirty: true });
                }}
                className="h-4 w-4 rounded accent-primary"
              />
              <label htmlFor="linkPurchaseInvoice" className="cursor-pointer text-sm font-medium text-text">
                ربط بفاتورة شراء أصلية (اختياري)
              </label>
            </div>
            ...
            {linkInvoice ? (
              <div>
                <SearchableSelect
                  value={Number(watch('purchase_invoice_id')) || null}
                  onChange={(id) => setValue('purchase_invoice_id', id ?? null, { shouldDirty: true })}
                  fetchFn={searchPurchaseInvoices}
                  queryKey="purchase-return-invoice"
                  placeholder="ابحث برقم الفاتورة..."
                  renderOption={(invoice) => `${invoice.invoice_number || `PI-${invoice.id}`} - ${formatCurrency(invoice.total_amount || 0)}`}
                  renderSelected={(invoice) => invoice.invoice_number || `PI-${invoice.id}`}
                  initialSelected={returnData?.invoice}
                />
                <input type="hidden" {...register('purchase_invoice_id')} />
              </div>
            ) : null}
          </div>
                      ...
                      <td className="px-3 py-2 align-top">
                        <SearchableSelect
                          ...
                          onChange={(id, variant) => {
                            ...
                            setValue(`items.${index}.unit_price`, Number(variant?.purchase_price ?? variant?.sale_price) || 0, {
                              shouldValidate: true,
                              shouldDirty: true,
                            });
                          }}
                          fetchFn={searchVariants}
                          queryKey={`purchase-return-variant-${index}`}
                          placeholder="ابحث عن منتج..."
                          renderOption={(variant) => `${variant.name} - ${formatCurrency(variant.purchase_price ?? variant.sale_price ?? 0)}`}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-text">المبلغ المسترد نقدًا</label>
              <input
                type="number"
                min="0"
                step="0.01"
              ...
              {errors.refund_amount?.message ? (
                ...
                <p className="text-xs text-text-muted">سيضاف للخزنة عند الحفظ</p>
              )}
            </div>
            ...
            <div className="space-y-2 rounded-lg bg-slate-50 p-4 text-sm">
              ...
              <div className="flex justify-between text-green-700">
                <span>يخصم من رصيد المورد</span>
                <span className="font-mono font-bold">- {formatCurrency(totals.totalAmount)}</span>
              </div>
              {refundAmount > 0 ? (
                <div className="flex justify-between text-primary">
                  <span>يضاف إلى الخزنة</span>
                  <span className="font-mono font-bold">+ {formatCurrency(refundAmount)}</span>
                </div>
              ) : null}
              {totals.remainingAmount > 0 ? (
                <div className="flex justify-between border-t border-border pt-2">
                  <span className="text-text-muted">مبلغ مستحق من المورد</span>
                  <span className="font-mono font-bold text-danger">{formatCurrency(totals.remainingAmount)}</span>
                </div>
              ) : null}
            </div>
          </div>
